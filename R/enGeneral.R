...
# .trim ------------------------------------------------------------------------
.trim <- function # remove spaces at begin and end of string(s)
### remove spaces at begin and end of string(s)
(
  x
) 
{
...
)
{
  ...
  plot(inpdat$COORDINATES$X_Coord, inpdat$COORDINATES$Y_Coord, 
       xlim = xlim, ylim = ylim, 
       asp=1, pch=pch, cex=cex, xlab=xlab, ylab=ylab, main=main, ...)
  ...
  if (!is.null(changedPipeIDs))
  {
    ...
    points(changedPipes$COORDINATES$X_Coord, 
           changedPipes$COORDINATES$Y_Coord, col="blue", pch=16)
    pipeXY <- getPipeCoordinates(changedPipes)
    segments(pipeXY$Node1.x, pipeXY$Node1.y, pipeXY$Node2.x, pipeXY$Node2.y, col="blue")
  }  
}
