...
#' @param returnOutput if TRUE, the output is read from the generated output file and returned
#' @param epanet.dir path to EPANET installation directory. 
#'   Default: getEpanetInstallationPath()
#' @param intern a logical, indicates whether to make the output of the command an R object.
#' @param write.output if TRUE, EPANET will write a binary output file, else not
#' @param \dots further arguments passed to \code{\link{readEpanetOutputFile}}, such as:
...
.epanetCommandLine <- function(epanet.exe, inpfile, rptfile, 
    outfile) {
    if (outfile != "") {
        outfile <- shQuote(outfile)
    }
    sprintf("\"%s %s %s %s\"", shQuote(epanet.exe), shQuote(inpfile), 
        shQuote(rptfile), outfile)
}
