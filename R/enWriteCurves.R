...
#' @param deleteOldCurves should all curves in 'epanetConfig' be deleted before writing?
#' 
...
#' @param deleteOldCurves if TRUE all curves in epantetConfig$CURVES will be deleted before adding
#'   new ones
#' 
#' @return Modified EPANET configuration  
#' 
...
.writeCurves <- function(epanetConfig, curvesTable, curveNamePrefix, 
    curveType, dataType, pattern = "", deleteOldCurves = FALSE, 
    dbg = TRUE) {
    ...
    required <- grep(pattern, epanetConfig[[sectionName]]$ID, 
        value = TRUE)
    .stopOnNoMatchingIDAtAll(provided = provided, required = required, 
        sectionName = sectionName, curveType = curveType, dataType = dataType, 
        dbg = dbg)
    epanetConfig <- .deleteCurvesIf(deleteOldCurves, epanetConfig)
    if (curveType == "efficiency") {
        epanetConfig$ENERGY <- .modifyEnergyConfiguration(energyConfig = epanetConfig$ENERGY, 
            provided = provided, curveNamePrefix = curveNamePrefix)
    }
    else {
        epanetConfig[[sectionName]] <- .modifyPumpOrValveSection(sectionConfig = epanetConfig[[sectionName]], 
            provided = provided, curveNamePrefix = curveNamePrefix, 
            curveType = curveType)
    }
    epanetConfig$CURVES <- .modifyCurvesConfiguration(curvesConfig = epanetConfig$CURVES, 
        curvesTable = curvesTable, curveNamePrefix = curveNamePrefix)
    epanetConfig
}

# ? ----------------------------------------------------------------------------

.modifyPumpOrValveSection <- function(sectionConfig, provided, 
    curveNamePrefix, curveType) {
    curvePrefix <- c(pump = "HEAD ", drawdown = "", efficiency = "")[curveType]
    columnName <- c(pump = "Parameters", drawdown = "Setting", 
        efficiency = "")[curveType]
    rows <- which(sectionConfig$ID %in% provided)
    sectionConfig[rows, columnName] <- paste(curvePrefix, curveNamePrefix, 
        sectionConfig$ID[rows], sep = "")
    sectionConfig
}

# ? ----------------------------------------------------------------------------

.modifyEnergyConfiguration <- function(energyConfig, provided, 
    curveNamePrefix) {
    ...
    if (all(columns.1.2 %in% columnNames)) {
        if (all(columns.1.4 %in% columnNames)) {
            ...
            restConfig <- energyConfig[which(!(is.efficiency & 
                is.provided)), ]
        }
        else {
            restConfig <- hsAddMissingCols(energyConfig, columns.3.4, 
                fill.value = "")
        }
    }
    else {
...
.configurePumpEfficiency <- function(pumpIDs, curveNamePrefix) {
    ...
    data.frame(X1 = rep("Pump", times = times), X2 = pumpIDs, 
        X3 = rep("Efficiency", times = times), X4 = sprintf("%s%s", 
            curveNamePrefix, pumpIDs), stringsAsFactors = FALSE)
}

# ? ----------------------------------------------------------------------------

.modifyCurvesConfiguration <- function(curvesConfig, curvesTable, 
    curveNamePrefix) {
    curvesTable$ID <- paste(curveNamePrefix, curvesTable$ID, 
        sep = "")
...
.stopOnMissingColumns <- function(availableColumns, curveType) {
    if (!all(c("ID", "X_Value", "Y_Value") %in% availableColumns)) {
        stop(.firstLetterUpperCase(curveType), " curve data not supplied in expected format!\n", 
            "data.frame(ID, X_VALUE, Y_VALUE)")
    }
}
...
.stopOnNoMatchingIDAtAll <- function(provided, required, sectionName, 
    curveType, dataType, dbg = TRUE) {
    ...
    if (noneDefined) {
        messageText <- paste(paste("No", objectName, "ID in section", 
            hsQuoteChr(sectionName), "defined that matches the names of the given", 
            curveType, "curves:"), paste(.firstLetterUpperCase(objectName), 
            "IDs in", hsQuoteChr(sectionName), "section:"), .spaceSeparatedAndCollapsed(required), 
    ...
    catIf(dbg, ifelse(noneDefined, "", paste("but without a", 
        curveType, "curve")))
    catIf(dbg, ":\n")
