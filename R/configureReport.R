...
#' 
#' set REPORT option in EPANET \code{configuration}
#' 
#' @param configuration EPANET \code{configuration}, representing an EPANET input \code{file}, as returned by 
#'   \code{\link{readEpanetInputFile}}
#' @param pagesize \code{pagesize} sets the number of lines written per page of the output report.
#'   The default is 0, meaning that no line limit per page is in effect. 
#' @param file \code{file} supplies the name of a \code{file} to which the output report will be
#'   written. If the \code{file} name contains spaces then it must be surrounded by
#'   double quotes. If not supplied then the Report \code{file}, as specified in the
#'   second parameter of the ENopen (or ENepanet) function will be used.
#' @param status \code{status} determines whether hydraulic \code{status} \code{messages} are written to the
#'   Report \code{file}. If YES is selected the \code{messages} will identify those network
#'   components that change \code{status} during each time step of the simulation. If
...
#'   is NO.
#' @param summary \code{summary} determines whether a \code{summary} table of number of network components
#'   and key analysis options is generated. The default is YES.
#' @param messages \code{messages} determines whether error and warning \code{messages} generated during a
#'   hydraulic/water quality analysis are written to the Report \code{file}. The
#'   default is YES.
#' @param energy \code{energy} determines if a table reporting average \code{energy} usage and cost for
#'   each pump is provided. The default is NO.
#' @param nodes \code{nodes} identifies which \code{nodes} will be reported on. You can either list
...
#' @param links \code{links} identifies which \code{links} will be reported on. You can either list
...
#' @param variables list of report \code{variables} as defined by \code{\link{reportVariable}}, e.g.
...
.setReportVariable <- function(section, variable) {
    ...
    newRows <- .addIfNotNull(newRows, variable$name, "BELOW", 
        variable$below)
    newRows <- .addIfNotNull(newRows, variable$name, "ABOVE", 
        variable$above)
    newRows <- .addIfNotNull(newRows, variable$name, "PRECISION", 
        variable$precision)
...
.addIfNotNull <- function(rows, variableName, propertyName, value) {
    if (!is.null(value)) {
        rows <- rbind(rows, data.frame(name = variableName, value = paste(propertyName, 
