...
#' @param warn if TRUE, a warning is given if no Node or Link results were found in the
#'   report file.
#' 
...
.extractDataOfType <- function(reportLines, objectType, warn = TRUE) {
    ...
    endindex <- min(indicesEmpty[indicesEmpty > startindex]) - 
        1
    ...
    dataFrame <- csvTextToDataFrame(textblock, fill = TRUE, col.names = col.names, 
        stringsAsFactors = FALSE)
    names(dataFrame) <- c(objectType, headerInfo$variableNames, 
        "Type")
    if (!is.statistic) {
        ...
        dataFrame[, c(time.columns, setdiff(names(dataFrame), 
            time.columns))]
    }
    else {
        headerRow <- hsTrim(reportLines[indices[1]])
        data.frame(Statistic = strsplit(headerRow, " ")[[1]][1], 
            dataFrame, stringsAsFactors = FALSE)
    }
}
...
.getBlockIndices <- function(starts, blocksize) {
    rep(starts, each = blocksize) + rep(seq_len(blocksize) - 
        1, length(starts))
}
...
.extractTimestamps <- function(reportLines, indices) {
    sapply(strsplit(reportLines[indices], split = " "), "[[", 
        6)
}
...
.hhmmssToSeconds <- function(x) {
    sapply(strsplit(x, ":"), FUN = function(x) sum(as.integer(x) * 
        c(3600, 60, 1)))
}
