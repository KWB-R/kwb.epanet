...
.getLinkOrNodeResults <- function(outdat, linkResults = TRUE, 
    objects, vars) {
    ...
    for (variable in vars) {
        ...
        resultBlock <- cbind(variable = varNameInfo[[variable]][1], 
            step = 1:nrow(valueMatrix), as.data.frame(valueMatrix))
...
.variableInfo <- function() {
    list(linkVariables = list(q = c("flow", "flows"), v = c("velocity", 
        "velocities"), hl = c("headloss", "headlosses"), wq = c("waterQuality", 
        "avgWaterQualities"), sta = c("status", "statusCodes"), 
        set = c("setting", "settings"), rr = c("reactionRate", 
            "reactionRates"), ff = c("frictionFactor", "frictionFactors")), 
        nodeVariables = list(d = c("demand", "demands"), h = c("head", 
            "heads"), p = c("pressure", "pressures"), wq = c("waterQuality", 
            "waterQualities")))
}
...
.getNodePropertyTimeSeries <- function(dynamicResults, property, 
    prolog) {
    .getNodeOrLinkPropertyTimeSeries(dynamicResults, property, 
        prolog, "Node")
}

# ? ----------------------------------------------------------------------------

.getLinkPropertyTimeSeries <- function(dynamicResults, property, 
    prolog) {
    .getNodeOrLinkPropertyTimeSeries(dynamicResults, property, 
        prolog, "Link")
}
...
.getNodeOrLinkPropertyTimeSeries <- function(dynamicResults, 
    property, prolog, nodeOrLink) {
...
.readEpilog <- function(con) {
    ...
    list(AvgBulkReactionRate = floatValues[1], AvgWallReactionRate = floatValues[2], 
...
.readDynamicResults <- function(con, prolog) {
    ...
    for (i in seq(1, by = 1, length.out = .numberOfPeriods(prolog))) {
        dynamicResults[[i]] <- .readDynamicResultsForOnePeriod(con, 
            prolog$NumberOfNodes, prolog$NumberOfLinks)
    }
    dynamicResults
}
...
.readDynamicResultsForOnePeriod <- function(con, numberOfNodes, 
    numberOfLinks) {
    list(NodeData = data.frame(DemandAtEachNode = .readDbl(con, 
        numberOfNodes), HeadAtEachNode = .readDbl(con, numberOfNodes), 
        PressureAtEachNode = .readDbl(con, numberOfNodes), WaterQualityAtEachNode = .readDbl(con, 
            numberOfNodes)), LinkData = data.frame(FlowInEachLink = .readDbl(con, 
        numberOfLinks), VelocityInEachLink = .readDbl(con, numberOfLinks), 
        HeadlossForEachLink = .readDbl(con, numberOfLinks), AvgWaterQualityInEachLink = .readDbl(con, 
            numberOfLinks), StatusCodeForEachLink = .readDbl(con, 
            numberOfLinks), SettingForEachLink = .readDbl(con, 
            numberOfLinks), ReactionRateForEachLink = .readDbl(con, 
            numberOfLinks), FrictionFactorForEachLink = .readDbl(con, 
            numberOfLinks)))
}
...
.readEnergyUseOfOnePump <- function(con) {
    data.frame(PumpIndexInListOfLinks = readBin(con, integer()), 
        PumpUtilization = readBin(con, double(), size = 4), AvgEfficiency = readBin(con, 
            double(), size = 4), AvgKwPerVolume = readBin(con, 
            double(), size = 4), AvgKW = readBin(con, double(), 
            size = 4), PeakKW = readBin(con, double(), size = 4), 
        AvgCostPerDay = readBin(con, double(), size = 4), PeakEnergyUsage = NA)
}

# ? ----------------------------------------------------------------------------

.readNumberOfStringsOfLength <- function(con, numberOfStrings, 
    stringLength) {
    strings <- character()
...
.readProlog <- function(con) {
    ...
    names(prolog) <- c("MagicNumber", "Version", "NumberOfNodes", 
    ...
    prolog$ChemicalConcentrationUnits <- .readStringOfLength(con, 
        32)
    prolog$IDStringOfEachNode <- .readNumberOfStringsOfLength(con, 
        prolog$NumberOfNodes, 32)
    prolog$IDStringOfEachLink <- .readNumberOfStringsOfLength(con, 
        prolog$NumberOfLinks, 32)
