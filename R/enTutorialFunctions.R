...
#' calculateSpecificEnergyDemand
#' 
#' @param ) list with elements \emph{Q}, \emph{E}, \emph{Eff}, holding the column
...
.findCombinedSchemes <- function(allSchemes, baseScheme, waterDemand, 
    COLNAMES) {
    allSchemes$timeFraction <- quotient(waterDemand - allSchemes[[COLNAMES$Q]], 
        baseScheme[[COLNAMES$Q]] - allSchemes[[COLNAMES$Q]])
    combiSchemes <- allSchemes[inRange(allSchemes$timeFraction, 
        0, 1), ]
    ...
    for (columnName in c(COLNAMES$Eff, COLNAMES$E)) {
        combiSchemes[[columnName]] <- .weightedAverage(weight1 = V1, 
            value1 = baseScheme[[columnName]], weight2 = V2, 
            value2 = combiSchemes[[columnName]])
    }
    combiSchemes$opSchemeID <- NA
    combiSchemes$nOn <- NA
    combiSchemes$PumpConfigName <- paste(baseScheme$PumpConfigName, 
        combiSchemes$PumpConfigName, sep = " & ")
...
.getPlotCharacters <- function(badQualityWells, configurationNames, 
    Q, demand) {
...
.replacePumpsInConfiguration <- function(configuration, newCurvesData, 
    pumpsToReplace) {
    pumpCurve <- .getCurveWithReplacedID(curveData = newCurvesData$Pump, 
        pumpsToReplace = pumpsToReplace, prefix = newCurvesData$PumpNamePrefix$PumpCurves)
    efficiencyCurve <- .getCurveWithReplacedID(curveData = newCurvesData$GlobalPumpEfficiency, 
        pumpsToReplace = pumpsToReplace, prefix = newCurvesData$PumpNamePrefix$GlobalPumpEfficiency)
    configuration$CURVES <- replaceCurves(curves = configuration$CURVES, 
...
.getCurveWithReplacedID <- function(curveData, pumpsToReplace, 
    prefix) {
...
.getEnergyPerPump <- function(epanetOutput, configuration, schemeID) {
    flowsInListForm <- .getFlowsInListForm(epanetOutput = epanetOutput, 
        configuration = configuration)
    .getExtendedEnergyReport(epanetOutput = epanetOutput, schemeID = schemeID, 
...
.getFlowsInListForm <- function(epanetOutput, configuration) {
    flows <- getLinkResults(epanetOutput, links = getNamesOfPumps(configuration), 
        vars = "q")
    hsMatrixToListForm(df = flows, keyFields = c("step", "variable"), 
        colNamePar = "Pump", colNameVal = "Q.m3.per.hour")
}

# ? ----------------------------------------------------------------------------

.getExtendedEnergyReport <- function(epanetOutput, schemeID, 
    flowsInListForm) {
    energyReport <- reportEnergyUse(epanetOutput)
    energyReport <- energyReport[energyReport$Percent.Utilization > 
        0, ]
    merge(data.frame(opSchemeID = schemeID, energyReport), flowsInListForm[flowsInListForm$step == 
        1, -(1:2)])
}

# ? ----------------------------------------------------------------------------

.energyPerPumpToTotalEnergy <- function(tmpEnergyPerPump, operationSchemes, 
    schemeID, COL.Q = "Q.m3.per.hour") {
    Q.sum <- sum(tmpEnergyPerPump[[COL.Q]], na.rm = TRUE)
    data.frame(opSchemeID = schemeID, PumpConfigName = operationSchemes$Label[operationSchemes$ID == 
        schemeID], Q.m3.per.hour.sum = Q.sum, Average.Efficiency.avg = quotient(sum(tmpEnergyPerPump$Average.Efficiency * 
        tmpEnergyPerPump[[COL.Q]]), Q.sum), Kw.hr.per.m3.avg = sum(quotient(tmpEnergyPerPump$Kw.hr.per.m3 * 
        tmpEnergyPerPump[[COL.Q]], Q.sum)), stringsAsFactors = FALSE)
}

# ? ----------------------------------------------------------------------------

.liveplot <- function(energyTotal, main, averageDemand) {
    plot(Kw.hr.per.m3.avg ~ Q.m3.per.hour.sum, data = energyTotal, 
        type = "p", pch = 16, main = main)
...
#' calibrateModel
#' 
#' @param configuration EPANET parameterisation, e.g. as retrieved by readEpanetInputFile()
#' @param pipeIDs regular expression or name of pipeID(s) to be used for calibration
#' @param measured measurement data for all pumps as data.frame e.g.
#'   data.frame(pumpNames=c("pmpW1", "pmpW2"), measuredQ=c(140,190))
#' @param pumpsToCalibrate regular expression or name of pumps to be used for calibration: e.g.
#'   "pmpW1"
#' @param parameterName name of ONE EPANET pipe parameters to be calibrated: e.g. `Diameter` or
#'   `Roughness`
#' @param parameterRange min/max range of possible calibration parameter values: 0-1; with 
#'   parameterRange*cun = newParameterValue, e.g. parameter range = 0.5 -> 50% 
#'   reduction of initial value of \code{parameterName} for all \code{pipeIDs} defined in 
#'   calibrateModel() 
#' @param showLivePlot current calibration status is plotted if showLivePlot=TRUE. Default: TRUE
#' @param \dots additional parameters to be passed to fitnessAdaptedModelConfiguration()
#' 
