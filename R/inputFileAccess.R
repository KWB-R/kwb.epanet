...
#' @param curveNames names of \code{curves} to be plotted. Default: all names in column \emph{ID}
#'   of \emph{curves}
...
#' @param inpfile full path to EPANET input file
#' 
#' @return character vector containing the names of the sections contained in the
...
#' 
#' @param inpfile full path to EPANET input file
#' @param sectionName name of section to be read, for possible section names see the 
...
.getSectionFromTextLines <- function(txtlines, sectionName) {
    ...
    if (length(headerRow) > 0) {
        ...
        captions <- .substSpecialCharacters(strsplit(header, 
            split = splitPattern)[[1]])
    ...
    if (length(body) > 0) {
        ...
        numberOfColumns <- max(c(length(captions), sapply(fields, 
            length)))
        fields <- lapply(fields, FUN = .fillupToLength, numberOfColumns)
        characterMatrix <- matrix(unlist(fields), ncol = numberOfColumns, 
            byrow = TRUE)
        result <- data.frame(characterMatrix, stringsAsFactors = FALSE)
    }
    else {
        numberOfColumns <- length(captions)
        result <- read.table(textConnection(""), col.names = .defaultColumnNames(numberOfColumns), 
...
#' curvesToText
#' 
#' @param curves data frame representing \code{curves}, with columns \emph{ID}, \emph{X_Value}, 
...
#'   \emph{curveName} are replaced with \code{x} and \code{y} values given in \emph{x} and
#'   \emph{y}, respectively
#' 
